...
static void CGTK_activate_contact(GtkListBox* box, GtkListBoxRow* row, gpointer user_data) {
	...
	if (strcmp(hdy_leaflet_get_visible_child_name(HDY_LEAFLET(gui->content_leaflet)), "chat\0") != 0) {
		hdy_leaflet_set_visible_child_name(HDY_LEAFLET(gui->content_leaflet), "chat\0");
	}

#ifdef HANDY_USE_ZERO_API
	gboolean unfolded = (hdy_leaflet_get_fold(HDY_LEAFLET(gui->content_leaflet)) == HDY_FOLD_UNFOLDED);
#else
	gboolean unfolded = !hdy_leaflet_get_folded(HDY_LEAFLET(gui->content_leaflet));
#endif
	
	if (unfolded) {
		gtk_widget_set_visible(gui->back_button, FALSE);
	}
}
...
void CGTK_init_contacts(GtkWidget* header, GtkWidget* content, cgtk_gui_t* gui) {
	...
	gui->add_button = gtk_button_new_from_icon_name("list-add-symbolic\0", GTK_ICON_SIZE_MENU);
	
	gui->identity_button = gtk_button_new_from_icon_name("user-info-symbolic\0", GTK_ICON_SIZE_MENU);
	gtk_widget_set_sensitive(gui->identity_button, FALSE);
	
	gtk_container_add(GTK_CONTAINER(header), gui->add_button);
	gtk_container_add(GTK_CONTAINER(header), gui->identity_button);
	
	gui->contacts_list = gtk_list_box_new();
	
	gtk_list_box_set_selection_mode(GTK_LIST_BOX(gui->contacts_list), GTK_SELECTION_BROWSE);
	gtk_widget_set_size_request(gui->contacts_list, 320, 0);
	gtk_widget_set_hexpand(gui->contacts_list, FALSE);
	gtk_widget_set_vexpand(gui->contacts_list, TRUE);
	
	GtkWidget* viewport = gtk_viewport_new(NULL, NULL);
	gtk_container_add(GTK_CONTAINER(viewport), gui->contacts_list);
	...
	g_signal_connect(gui->add_button, "clicked\0", G_CALLBACK(CGTK_add_contact_dialog), gui);
	g_signal_connect(gui->identity_button, "clicked\0", G_CALLBACK(CGTK_open_identity), gui);
